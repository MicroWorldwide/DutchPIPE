...
?>
var dpclient_url = '<?php echo DPSERVER_CLIENT_URL; ?>';
...
function load_xml(xml)
{
    if (window.DOMParser) {
        ...
        alert('<?php echo dptext('Failure loading XML'); ?>');
    } else {
        alert('<?php echo dptext('Your browser cannot handle this script'); ?>');
    }
}
...
function handle_dp_element(childnode)
{
    ...
    default:
        if (inner_text(childnode) != '1') {
            alert('<?php echo dptext('Invalid DutchPIPE XML (2), tagname:'); ?>' + childnode.tagName);
        }
    }
}
...
function start_dutchpipe(page)
{
    ...
    var dphost_url = '<?php echo DPSERVER_HOST_URL; ?>';
