 ...
 * @version    Subversion: $Id: dpclient-js.php 241 2007-07-08 15:59:39Z ls $
...
function get_actions(id, event)
{
    ...
    for (i = 1; i <= am_level; i++) {
        lvls += '&l' + i + '=' + escape(arguments[i + 1]);
        am_levels[i - 1] = escape(arguments[i + 1]);
    }
...
function action_over(el)
{
    if (am_level != undefined && am_levels != undefined) {
        ...
        if (lvl != am_level) {
            if (jQuery(el).text() != am_levels[lvl]) {
                jQuery('div.am_deep_selected').
...
function action_out(el)
{
    jQuery(el).removeClass('am_selected');
}

function get_map_area_actions(map_name, map_area_id, id, event)
{
    var lvl = am_level = arguments.length - 4;
    var lvls = '';
    var i;

    am_levels = new Array(lvl);
...
dptext('You must have (session) cookies enabled in order to view this site');
                ?></h1>\n";
        }
    ...
    case 'event':
        ...
        if (event_count != null && event_count != -1
            ...
            alert('<?php echo dptext('Invalid event count'); ?> ' + event_count
                 + ' (received ' + e_count + '): ' + jQuery(childnode).text());
...
function handle_dp_event(childnode)
{
    ...
    case 'div':
        ...
        var newdiv = jQuery(document.createElement("DIV")).attr('id',
            jQuery(childnode).attr('id')).html(tmp).css('zIndex', 5);
    ...
    case 'message':
        ...
        if (message == 'close_window') {
            this.location = '<?php echo DPSERVER_CLIENT_DIR;
                ?>dpmultiplewindows.txt';
            return;
        }
        ...
        if (messagediv.childNodes.length == 16) {
            jQuery(messagediv).css({ width: '100%', height: jQuery(messagediv).
    ...
    case 'addDpElement':
        if (jQuery('#' + jQuery(childnode).attr('where')).length)
            jQuery(document.createElement("div")).attr('id', jQuery(childnode).
                attr('id')).addClass(jQuery(childnode).attr('class')).
                html(jQuery(childnode).text()).appendTo(jQuery('#'
                + jQuery(childnode).attr('where')));
        break;
    case 'changeDpElement':
        jQuery('#' + jQuery(childnode).attr('id')).html(jQuery(childnode).
            text());
        break;
    case 'moveDpElement':
        jQuery('#' + jQuery(childnode).attr('id')).removeClass().remove().
            clone().attr('id', jQuery(childnode).attr('id')).
            addClass(jQuery(childnode).attr('class')).appendTo(jQuery('#'
            + jQuery(childnode).attr('where')));
        break;
    case 'window':
        make_dpwindow(jQuery(childnode).text(), jQuery(childnode).
            attr('autoclose'), jQuery(childnode).attr('styleclass'));
        break;
    case 'reportmove':
        jQuery('#' + jQuery(childnode).attr('id')).animate({ 'left' :
            parseInt(jQuery(childnode).attr('left'))}).animate({ 'top' :
            parseInt(jQuery(childnode).attr('top'))});
        break;
    case 'history':
    ...
    default:
        break;
        if (jQuery(childnode).text() != '1')
            alert('<?php echo dptext('Invalid DutchPIPE XML (1), tagname:'); ?>'
                + childnode.tagName);
    }
}
...
function dbg(str)
{
    jQuery('div#debug').html(str + '<br />' + jQuery('div#debug').html());
}
...
function make_amenu(el)
{
    ...
    var lft = !level ? (am_mouse_x-11) : parseInt(e.css('left')) + e.width()
        - 5;
    var tp = !level ? (am_mouse_y-13) : parseInt(e.css('top')) + (15 * indx);
    var scrolltop = jQuery.boxModel && document.documentElement.scrollTop ||
        document.body.scrollTop;
    var amenu = jQuery(document.createElement("div")).attr('id', 'am_div'
        + level). addClass('actionwindow').html(jQuery(el).text()).
        css({ left: lft+'px', top: tp+'px' });

    amenu.appendTo(document.body);
    var zindex = parseInt(amenu.css('z-index'));
    amenu.css('z-index', -1).css('visiblity', 'hidden').css('display', 'block');
    var w = amenu.get(0).clientWidth;
    amenu.css('display', 'none').css('visiblity', 'visible').css('z-index',
        zindex);
    if (w < 96) {
        amenu.css('width', '96px');
        w = 96;
    }
    if (w > 200) {
        amenu.css('width', '200px');
        w = 200;
    }

    if (lft + w + 15 > wwidth())
        amenu.css('left', (lft - (!level ? w - 21 : (2 * w) - 5)) + 'px');
    if (tp + (8 + 15 * jQuery('.am', amenu).length) - scrolltop > wheight())
        amenu.css('top', (tp - (8 + 15 * jQuery('.am', amenu).length) + 23)
        + 'px');
    ...
    if (jQuery.browser.msie) {
        jQuery('.am .am_deep .am_selected .am_deep_selected').css(
            'text-overflow', 'ellipsis');
        if (amenu.css('-moz-opacity'))
            amenu.css('filter', 'alpha(opacity='
                + (amenu.css('-moz-opacity') * 100) + ')');
        else if (amenu.css('opacity') == '1') // IE6
            amenu.css('filter', 'alpha(opacity=95)');
    }
}
...
function make_dpwindow(text, autoclose, styleclass)
{
    ...
    jQuery(dpwindow).addClass(styleclass == null ? 'dpwindow_default'
        : styleclass);

    if (autoclose == null)
        jQuery(dpwindow).html(text + '<p align="right">'
            + '<a href="javascript:close_dpwindow()"><?php echo dptext('close');
            ?></a></p>');
    else {
    ...
    if (jQuery.browser.msie) {
        if (jQuery(dpwindow).css('-moz-opacity'))
            jQuery(dpwindow).css('filter', 'alpha(opacity='
                + (jQuery(dpwindow).css('-moz-opacity') * 100) + ')');
        else if (jQuery(dpwindow).css('opacity') == '1') // IE6
            jQuery(dpwindow).css('filter', 'alpha(opacity=95)');
    }
